...
export class CreatePurchaseReturnComponent implements OnInit {
  ...
  constructor(public poservice: CreatePoService,private commonService:CommonService,
    private router:Router,private prs:CreatePRService) {
}
  toggle1() {
    this.show1 = true;
  }
...
reqfield(city,warehouse,vendor){
 ...
 public goselect(){
  ...
  public vendordetail(e){
    this.poservice.vendorDetail({ "requestedId": e.target.value,
...
// ****************for final create po submit*************************
  public submitPO() {
   ...
   if(this.pRODetailRequestViewModels.length!=0)
   {
    ...
    this.prs.createPRO(this.data).subscribe(res => {
