...
import{EditproService} from '../Core/editpro.service';
import {CreatePoService}from '../Core/create-po.service';
import {CreatePRService} from '../Core/create-pr.service';
...
export class EditproComponent implements OnInit {
  ...
  constructor( private route: ActivatedRoute,private commonService:CommonService,
    private router: Router, private prs:CreatePRService,private edits:EditproService,private poservice:CreatePoService) { }
    toggle1() {
      this.show1 = true;
    }
  ...
  ngOnInit() {
    ...
    this.prodetail({"requestedId": this.proId,
    "userId": 1});
...
{
  this.edits.getprodetail(data).subscribe(res=>{
...
this.selectedfinallist.forEach((item)=>{
  if(this.addeditem.filter(data=>data.id==item.id).length==0){
    ...
    this.createDataForPROinsert();
      ...
      this.edits.addproitem(this.data2).subscribe(res => {
...
this.updateDataForPROCreation();
 ...
 { 
  ...
  this.edits.updateprodetail(this.data).subscribe(res => { 
    if(!res.didError)
    {
...
public release() {
  this.released = true;
  this.edits.releaseprodetail({ requestedId: this.proId, userId: 1 }).subscribe(res => { 
