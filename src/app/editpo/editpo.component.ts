...
export class EditpoComponent implements OnInit {
  ...
  constructor( private route: ActivatedRoute,
    private router: Router,private edits:EditpoService,private poservice:CreatePoService,private commonservice:CommonService ) { }
    toggle1() {
      this.show1 = true;
    }
    toggle2() {
  ...
  ngOnInit() {
    // alert(this.today);
    this.commonservice.setTitle('IMS-Warehouse-Inbound-Edit PO');
...
{
  this.edits.getpodetail(data).subscribe(res=>{
...
public pocancle(){
  this.edits.canclepodetail({
    "requestedId": this.poId,
    "userId": 1
  }).subscribe(res=>{
    console.log(res,"mlmlmlmlml");
    if(!res.didError){
      swal({
        title: "Success!",
        text:"Po cancle successfully",
        type: "success",
        showCancelButton: false,
        confirmButtonText: 'OK'
      });
...
closename(){
 ...
 // ****************for polist selector*************************
public polist() {
 // console.log(this.wareId, this.vendorId, "daa");
  this.poservice.polist({ "userId": "1" , "whId": this.wareId, "vId": this.vendorId, }).subscribe(res => {
   //console.log(res.model,"pooooooooooooooooooooooooooooooo");
    this.po = res.model;
...
this.edits.addpoitem(this.data2).subscribe(res => {
...
//  console.log(this.poUpdateDetails," final jo jayega podetails");
 if(this.poUpdateDetails.length!=0)
 {
  ...
  this.edits.updatepodetail(this.data).subscribe(res => {
