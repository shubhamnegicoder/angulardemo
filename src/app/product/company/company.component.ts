...
export class CompanyComponent implements OnInit {
  ...
  constructor(private pageService: PageService,private commonService:CommonService, private companyServiceObj: CompanyService, private importExcelService: ImportService, private downloadService: DownloadService) { }
  ...
  public getCompanyList(data) {
    ...
    this.companyServiceObj.getCompanyList(data).subscribe(res => {
  ...
  createCompany(compName) {
    ...
    if (compName === "") {
      ...
      this.companyServiceObj.createCompany(this.dataForCompCreation).subscribe(response => {
  ...
  editCompany(compName) {
    console.log("company to be edited:-> " + compName);
    //need to call the service from here to create brand.
    if (compName === "") {
      swal({
        title: '',
      ...
      console.log("data to update:-> " + this.dataForCompCreation.name, this.dataForCompCreation.id, this.dataForCompCreation.status);

      this.companyServiceObj.updateCompany(this.dataForCompCreation).subscribe(response => {
  ...
  changeStatus(id) {
    ...
    swal({
      ...
      if (result.value) {
        ...
        this.companyServiceObj.changeStatus(objToDel).subscribe(res => {
  ...
  importAsXLSX() {
    ...
    if (this.fileToUpload) {
      this.importExcelService.uploadBrand(this.fileToUpload, 1, 'api/Company/Import').subscribe(res => {
